...
export default class RemodalService extends Service {
  ...
  open(
    ...
    if (opts) {
      // Matches the old addon's setProperties behavior: overrides persist
      // across subsequent opens until replaced.
      modal.serviceOverrides = opts;
    }
    return modal.open();
  }

  close(name = 'ember-remodal'): Promise<EmberRemodal> {
    return this.lookup(name).close();
  }

  private lookup(name: string): EmberRemodal {
