...
const FOCUSABLE_SELECTOR = ['a[href]', 'area[href]', 'button', 'details', 'summary', 'iframe', 'input', 'select', 'textarea', 'audio[controls]', 'video[controls]', '[contenteditable]:not([contenteditable="false"])', '[tabindex]'].join(',');
function hasFocusableDescendant(root) {
  return root !== null && root.querySelector(FOCUSABLE_SELECTOR) !== null;
}
const MISSING_FOCUSABLE_CONTENT_MESSAGE = 'ember-remodal: the yielded <m.open> / <m.confirm> / <m.cancel> components render a click-delegating <span>, so the block must contain your own focusable control. `<m.open>Open modal</m.open>` produces a trigger that mouse users can click but keyboard users can never reach (WCAG 2.1.1). Wrap the label in a real control: `<m.open><button type="button">Open modal</button></m.open>`.';
class ErButton extends Component {
  handleClick = event => {
    ...
    void Promise.resolve(this.args.onClick(event)).catch(error => {
      console.error(error);
    });
  };
  ...
  auditFocusableContent = modifier(element => {
    let cancelled = false;
    if (!hasFocusableDescendant(element)) {
      void Promise.resolve().then(() => {
        ...
        warn(MISSING_FOCUSABLE_CONTENT_MESSAGE, hasFocusableDescendant(element), {
...
export { ErButton as default, hasFocusableDescendant };
//# sourceMappingURL=er-button.js.map
