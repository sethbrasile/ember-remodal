import { resetScrollLockForTesting, setAnimationDisabledForTesting, scrollLockStateForTesting } from '../components/ember-remodal.js';
...
const FORCE_RESET_NOTE = 'It has been force-released, so the tests after this one are unaffected — this failure is the leak itself, not a consequence of one.';
 ...
 * hooks yourself.
 */
function resetRemodalScrollLock() {
 ...
 */
function setRemodalAnimationDisabled(disabled) {
 ...
 */
function setupRemodal(hooks, options = {}) {
  const {
  ...
  hooks.afterEach(function (assert) {
    ...
    const accountedFor = remodalDialogs().some(element => element.open || !element.classList.contains('remodal-is-closed'));
...
function resolveScope(scope = document) {
  ...
  if (!root) {
    throw new Error(`ember-remodal/test-support: no element matched the scope selector "${scope}".`);
  }
  return root;
}
 ...
 */
function remodalDialog(scope) {
  const dialogs = remodalDialogs(scope);
  if (dialogs.length === 0) {
    throw new Error('ember-remodal/test-support: no modal <dialog> is rendered (looked for `[data-test-id="modalWrapper"]`).');
  }
  if (dialogs.length > 1) {
    throw new Error(`ember-remodal/test-support: ${dialogs.length} modal <dialog> elements are rendered. Scope the lookup (remodalDialog('[data-test-id="my-modal"]')) or use remodalDialogs().`);
  }
  return dialogs[0];
}

export { remodalDialog, remodalDialogs, resetRemodalScrollLock, setRemodalAnimationDisabled, setupRemodal };
